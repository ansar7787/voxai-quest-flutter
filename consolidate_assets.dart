...
void main() async {
  ...
  for (var entry in gameMap.entries) {
    ...
    for (var game in games) {
      final prefix = game[0];
      final List<dynamic> allQuests = [];

      debugPrint('Processing $category/$prefix...');
        ...
        if (await file.exists()) {
          try {
            ...
            if (data.containsKey('quests')) {
              allQuests.addAll(data['quests'] as List);
            }
          } catch (e) {
      ...
      if (allQuests.isNotEmpty) {
        final consolidatedData = {'gameType': game[1], 'quests': allQuests};

        final outPath = '$basePath/$category/$prefix.json';
        await File(
          outPath,
        ).writeAsString(JsonEncoder.withIndent('  ').convert(consolidatedData));
        debugPrint('  Consolidated ${allQuests.length} quests into $outPath');
      } else {
        debugPrint('  No quests found for $prefix');
      }
    }
  }

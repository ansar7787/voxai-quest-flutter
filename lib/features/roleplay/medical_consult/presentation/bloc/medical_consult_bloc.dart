...
class MedicalConsultBloc extends Bloc<MedicalConsultEvent, MedicalConsultState> {
  final GetMedicalConsultQuests getQuests;

  MedicalConsultBloc({required this.getQuests}) : super(MedicalConsultInitial()) {
  ...
  Future<void> _onFetchQuests(
    ...
    result.fold(
      (failure) => emit(const MedicalConsultError("Failed to load medical consult quests")),
  ...
  void _onNextQuestion(
    ...
    if (state is MedicalConsultLoaded) {
      if (state.currentIndex + 1 < state.quests.length) {
        ...
        emit(MedicalConsultGameComplete(

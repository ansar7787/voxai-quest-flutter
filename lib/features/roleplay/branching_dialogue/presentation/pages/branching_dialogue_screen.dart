...
class BranchingDialogueScreen extends StatefulWidget {
  final int level;
  const BranchingDialogueScreen({super.key, required this.level});
...
class _BranchingDialogueScreenState extends State<BranchingDialogueScreen> {
  ...
  void _onOptionSelected(int index, String optionText) {
    ...
    if (state is RoleplayLoaded) {
      // For simplicity in this specialized game, we assume 3 steps of dialogue.
      // But we use the bloc's SubmitAnswer logic for correctness validation.
      // Usually, roleplay options have one "best" answer (index 0 in mock)
        ...
        listener: (context, state) {
          if (state is RoleplayGameComplete) {
            ...
            if (_chatMessages.isEmpty || !_chatMessages.last['isUser']) {
              // Add AI's first message if not present
              _chatMessages.add({
                'text': state.currentQuest.roleName != null
                    ? "Hi! I am ${state.currentQuest.roleName}. Let's discuss this scenario."
        ...
        if (state.lastAnswerCorrect != null)
          ModernGameResultOverlay(
            ...
            title: state.lastAnswerCorrect!
                ...
                : "SOCIAL SLIP!",
            subtitle:
                "Best Response: ${quest.options?[0] ?? "The appropriate response."}",
  ...
  void _showCompletionDialog(BuildContext context, int xp, int coins) {
    ...
    showDialog(
      ...
      builder: (c) => ModernGameDialog(
        ...
        description:
            'You earned $xp XP and $coins Coins for your social grace!',
        buttonText: 'OK',
        onButtonPressed: () {
          Navigator.pop(c);
          context.pop();
  ...
  void _showGameOverDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (c) => ModernGameDialog(
        ...
        description:
            'The conversation came to an abrupt halt. Recharge and try again!',
        buttonText: 'QUIT',
        isSuccess: false,
        onButtonPressed: () {
          Navigator.pop(c);

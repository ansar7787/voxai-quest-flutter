...
class TopicVocabBloc extends Bloc<TopicVocabEvent, TopicVocabState> {
  ...
  Future<void> _onFetchQuests(
    ...
    result.fold((failure) => emit(TopicVocabError(failure.message)), (quests) {
      if (quests.isEmpty) {
  ...
  void _onHintUsed(TopicVocabHintUsed event, Emitter<TopicVocabState> emit) {
    final state = this.state;
    if (state is TopicVocabLoaded) {
